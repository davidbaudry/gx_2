...
function getGameFromId($game_id)
{
    ...
    $q = '
        ...
        INNER JOIN authors a ON(a.id = g.author_id)
        INNER JOIN editors e ON(e.id = g.editor_id)
        WHERE g.id = "'.Securite::bdd($game_id).'";';
...
function getGameTopScores($game_id, $top = 3)
{
    ...
    $q = '
        ...
        WHERE g.game_id = '.Securite::bdd($game_id).'
        ORDER BY score DESC
        LIMIT '.Securite::bdd($top).';';
...
function insertGame($data) {
    ...
    $q = 'INSERT INTO `games` '
            ...
            . 'VALUES '
            . '(NULL, '
            . '"' . Securite::bdd($data['name']) . '",
            "' . Securite::bdd($data['is_extension']) . '", 
            "' . Securite::bdd($data['author_id']) . '",
            "' . Securite::bdd($data['editor_id']) . '",
            "' . Securite::bdd($data['is_collaborative']) . '",
            "' . Securite::bdd($data['has_invert_score']) . '",
            "' . Securite::bdd($data['description']) . '");';
    //echo $q;
...
function insertPlayer($data) {
    ...
    $q = 'INSERT INTO `players` '
            ...
            . 'VALUES '
            . '(NULL, '
            . '"' . Securite::bdd($data['firstname']) . '",
            "' . Securite::bdd($data['lastname']) . '");';
    //echo $q;
    return $database->query($q);
}
...
function getPlayerFromId($player_id)
{
    ...
    $q = '
        SELECT id, firstname, lastname
        FROM players
        WHERE id = ' . Securite::bdd($player_id) . ';';
...
function getNbPlayedGamesByPlayer($player_id)
{
    ...
    $q = '
        ...
        WHERE `player_id` = ' . Securite::bdd($player_id) . '
        GROUP BY g.id
        ORDER BY C DESC
        LIMIT 1000';
...
function getInfoGamePlayer($game_id, $player_id)
{
    ...
    foreach ($gameplays as $gameplay)
    {
        ...
        $q_meilleur_score = '
            ...
            WHERE `gameplay_id` = ' . Securite::bdd($gameplay->id) . ' 
            ORDER BY `score` DESC 
            LIMIT 1';
...
function getGameplayFromId($gameplay_id)
{
    ...
    $q = '
            ...
            WHERE gp.id = ' . Securite::bdd($gameplay_id) . '
...
function getGameplaysListFromGameid($game_id)
{
    ...
    $q = '
            ...
            FROM gameplay gp 
            WHERE gp.game_id = ' . Securite::bdd($game_id) . '
...
function getPlayersFromGameplayId($gameplay_id)
{
    ...
    $q = '
            ...
            FROM gameplay_lines gpl
            INNER JOIN players p ON (p.id = gpl.player_id)
            WHERE gpl.gameplay_id = ' . Securite::bdd($gameplay_id) . '
            ORDER BY score ASC';
...
function insertGameplay($data)
{
    ...
    $q = 'INSERT INTO `gameplay` (`id`, `date`, `game_id`) VALUES (NULL,  
        "' . Securite::bdd($data['date']) . '", 
        "' . Securite::bdd($data['game_id']) . '");';
    //echo $q;
    return $database->query($q);
...
function insertGameplayLine($gameplay_id, $player_id, $score)
{
    ...
    $q = 'INSERT INTO `gameplay_lines` (`id`, `gameplay_id`, `player_id`, `score`) 
        VALUES (NULL, 
        "' . Securite::bdd($gameplay_id) . '", 
        "' . Securite::bdd($player_id) . '", 
        "' . Securite::bdd($score) . '"
        );';
    //echo $q;
    return $database->query($q);
...
function getNbPlayedGameplays($game_id)
{
    global $database;
    $q = '
            SELECT COUNT(`id`) AS C FROM `gameplay` WHERE `game_id` = ' . Securite::bdd($game_id) . ';';
...
function getNbPlayedGameplaysByPlayers($player_id)
{
    global $database;
    $q = '
            SELECT COUNT(`id`) AS C FROM `gameplay_lines` WHERE `player_id` = ' . Securite::bdd($player_id) . ';';
    $nb = $database->get_row($q, true);
    return $nb;
}
